...
interface EditAssetModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (id: string, data: UpdateAssetRequest) => Promise<boolean>;
  asset: UserAsset | null;
  categories: AssetCategory[];
  loading?: boolean;
}

interface FormData extends UpdateAssetRequest {
  purchaseDate?: string;
...
const EditAssetModal: React.FC<EditAssetModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen && asset) {
      const formData: FormData = {
        ...
        annualReturn: asset.annualReturn || undefined,
        marketValue: asset.marketValue || undefined,
        valuationDate: asset.valuationDate ? new Date(asset.valuationDate).toISOString().split('T')[0] : '',
      ...
      setShowAdditionalInfo(Boolean(asset.additionalInfo && Object.keys(asset.additionalInfo).length > 0));
    }
  }, [isOpen, asset, reset]);
  ...
  const handleFormSubmit = async (data: FormData) => {
    ...
    try {
      logger.debug('Submitting edit asset form', { assetId: asset.id, data });
      ...
      const submitData: UpdateAssetRequest = {
        ...data,
      ...
      if (success) {
        logger.info('Asset updated successfully', { assetId: asset.id });
        onClose();
      }
    } catch (error) {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Cập nhật thông tin chi tiết về tài sản
                </p>
              </div>
            </div>
            <button
          ...
          <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
            ...
            <div className="flex items-center justify-end gap-3 pt-6 border-t border-gray-200">
              ...
              >
                ...
                {isSubmitting || loading ? 'Đang cập nhật...' : 'Cập nhật tài sản'}
              </button>
            </div>
          </form>
        </div>
